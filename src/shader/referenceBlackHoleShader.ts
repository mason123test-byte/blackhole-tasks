...
fragment = replaceOnce(fragment, "  bool captured = false; int diskCrossingCount = 0; vec3 accumulatedDisk = vec3(0.0); float transmittance = 1.0;", "  bool captured = false; int diskCrossingCount = 0; vec3 accumulatedDisk = vec3(0.0); float transmittance = 1.0; float pathLength = 0.0; float diagnosticCoverage = 0.0;");
...
fragment = replaceOnce(fragment, "  if (diskCrossingCount > 0) {", "  if (crossingDiagnosticMode > 0.5) {\n    outColor = vec4(accumulatedDisk, clamp(diagnosticCoverage, 0.0, 1.0));\n    return;\n  }\n  if (diskCrossingCount > 0) {");
