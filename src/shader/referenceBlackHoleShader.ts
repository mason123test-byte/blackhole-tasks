...
float tangentSymplecticCrossCoupling(
  float r,
  ...
  float normalization = max(
    sqrt((dot(qRight, qRight) + dot(pRight, pRight)) * (dot(qUp, qUp) + dot(pUp, pUp))),
    1e-6
  );
  return clamp(omega / normalization, -1.0, 1.0);
}
...
void shapeIncidenceQualifiedDirectShoulder(
  ...
  float symplecticCancellationSupport,
