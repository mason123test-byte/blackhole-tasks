...
bool includeCrossingInDiagnostic(float diagnosticMode, int crossingIndex) {
  ...
  if (diagnosticMode < 3.5) return crossingIndex >= 2;
  return false;
}

vec3 terminationDiagnosticColor(float reason) {
  if (reason < 1.5) return vec3(0.50, 0.0, 0.0); // horizon
  if (reason < 2.5) return vec3(0.0, 0.50, 0.0); // escape
  if (reason < 3.5) return vec3(0.0, 0.0, 0.50); // max steps
  if (reason < 4.5) return vec3(0.50, 0.50, 0.0); // transmittance cutoff
  if (reason < 5.5) return vec3(0.50, 0.0, 0.50); // integrator rejection
  return vec3(0.0, 0.50, 0.50); // pre-trace camera-impact cull
}
`;
...
fragment = replaceOnce(fragment, "uniform float u_visual_compare;\n", `uniform float u_visual_compare;\n${experimentUniforms}`);
fragment = replaceOnce(fragment, "const float DISK_SOURCE_DIAGNOSTIC = 0.0;\n", `const float DISK_SOURCE_DIAGNOSTIC = 0.0;\n\n${experimentHelpers}`);
fragment = replaceOnce(fragment, "\nvoid rayTracedReference() {", `${incidenceHelpers}\nvoid rayTracedReference() {`);
fragment = replaceOnce(fragment, "  float candidateWeight = u_visual_compare < 0.5 ? 0.0 : (u_visual_compare > 1.5 ? step(0.0, screen.x) : 1.0);", "  float splitWeight = 1.0 - step(0.5, abs(u_visual_compare - 2.0));\n  float candidateWeight = u_visual_compare < 0.5 ? 0.0 : mix(1.0, step(0.0, screen.x), splitWeight);\n  float crossingDiagnosticMode = crossingOrderDiagnosticMode();");
fragment = replaceOnce(fragment, "  float outerEdge = 1.0 - smoothstep(DISK_OUTER * 0.74, DISK_OUTER * 0.995, hitRadius);", "  float experimentDiskOuter = visualExperimentDiskOuter();\n  float outerEdge = 1.0 - smoothstep(experimentDiskOuter * 0.74, experimentDiskOuter * 0.995, hitRadius);");
fragment = replaceOnce(fragment, "  float radialProgress = clamp((hitRadius - DISK_INNER) / (DISK_OUTER - DISK_INNER), 0.0, 1.0);", "  float radialProgress = clamp((hitRadius - DISK_INNER) / (experimentDiskOuter - DISK_INNER), 0.0, 1.0);");
fragment = replaceOnce(fragment, "  float cameraImpact = OBSERVER_R * length(cameraPlane);\n  if (cameraImpact > DISK_OUTER + 10.0) { outColor = sceneSample; return; }", "  float experimentDiskOuter = visualExperimentDiskOuter();\n  float cameraImpact = OBSERVER_R * length(cameraPlane);\n  if (cameraImpact > experimentDiskOuter + 10.0) {\n    outColor = crossingDiagnosticMode > 5.5 ? vec4(terminationDiagnosticColor(6.0), 1.0) : (crossingDiagnosticMode > 0.5 ? vec4(0.0) : sceneSample);\n    return;\n  }");
fragment = replaceOnce(fragment, "      if (diskRadius > DISK_INNER && diskRadius < DISK_OUTER) {", "      if (diskRadius > DISK_INNER && diskRadius < experimentDiskOuter) {");
fragment = replaceOnce(fragment, "  brightness *= physicalLayers;\n  brightness *= mix(1.10, 0.62, smoothstep(0.04, 0.90, radialProgress)); brightness *= FILM_DISK_EXPOSURE;", "  brightness *= physicalLayers;\n  brightness *= mix(1.10, 0.62, smoothstep(0.04, 0.90, radialProgress)); brightness *= visualExperimentFilmDiskExposure();");
fragment = replaceOnce(fragment, "  bool captured = false; int diskCrossingCount = 0; vec3 accumulatedDisk = vec3(0.0); float transmittance = 1.0;", "  bool captured = false; int diskCrossingCount = 0; vec3 accumulatedDisk = vec3(0.0); float transmittance = 1.0; float pathLength = 0.0; float diagnosticCoverage = 0.0; float diagnosticThirdReached = 0.0; vec3 diagnosticThirdPreTrans = vec3(0.0); float diagnosticThirdPreCoverage = 0.0; float diagnosticTerminationReason = 3.0;");
fragment = replaceOnce(fragment, "    if (r <= KERR_HORIZON + 0.015) { captured = true; break; }", "    if (r <= KERR_HORIZON + 0.015) { captured = true; diagnosticTerminationReason = 1.0; break; }");
fragment = replaceOnce(fragment, "    if (r > OBSERVER_R + 6.0 && dr0 > 0.0) break;", "    if (r > OBSERVER_R + 6.0 && dr0 > 0.0) { diagnosticTerminationReason = 2.0; break; }");
fragment = replaceOnce(fragment, "    if (!acceptedStep) {\n      break;\n    }", "    if (!acceptedStep) {\n      diagnosticTerminationReason = 5.0;\n      break;\n    }");
fragment = replaceOnce(fragment, "    h = clamp(h * clamp(0.90 * pow(max(acceptedErrorRatio, 1e-6), -0.20), 0.55, 1.80), KERR_MIN_STEP, KERR_MAX_STEP);", "    float acceptedStepLength = h;\n    h = clamp(h * clamp(0.90 * pow(max(acceptedErrorRatio, 1e-6), -0.20), 0.55, 1.80), KERR_MIN_STEP, KERR_MAX_STEP);");
fragment = replaceOnce(fragment, "        sampleDiskSurface(diskRadius, diskPhi, patternTime, diskColor, diskAlpha);\n        float colorGain = crossingColorGain(diskCrossingCount);", `        sampleDiskSurface(diskRadius, diskPhi, patternTime, diskColor, diskAlpha);\n        if (diskCrossingCount == 0) {\n          float hitPathLength = pathLength + crossing * acceptedStepLength;\n          float radialDirectDistance = max(OBSERVER_R - diskRadius, 1.0);\n          float pathStretch = hitPathLength / radialDirectDistance;\n          float incidenceCosine = diskLocalIncidenceCosine(diskRadius, L, kappa);\n          shapeIncidenceQualifiedDirectShoulder(candidateWeight, pathStretch, incidenceCosine, diskColor, diskAlpha);\n        }\n        float colorGain = crossingColorGain(diskCrossingCount);`);
fragment = replaceOnce(fragment, "        float effectiveAlpha = clamp(diskAlpha * alphaGain, 0.0, 0.90);\n        accumulatedDisk += transmittance * (diskColor * colorGain) * effectiveAlpha;\n        transmittance *= 1.0 - effectiveAlpha;", "        float effectiveAlpha = clamp(diskAlpha * alphaGain, 0.0, 0.90);\n        float incomingTransmittance = transmittance;\n        vec3 preTransContribution = (diskColor * colorGain) * effectiveAlpha;\n        if (diskCrossingCount >= 2) {\n          diagnosticThirdReached = 1.0;\n          diagnosticThirdPreTrans += preTransContribution;\n          diagnosticThirdPreCoverage += effectiveAlpha;\n        }\n        if (includeCrossingInDiagnostic(crossingDiagnosticMode, diskCrossingCount)) {\n          accumulatedDisk += incomingTransmittance * (diskColor * colorGain) * effectiveAlpha;\n          diagnosticCoverage += incomingTransmittance * effectiveAlpha;\n        }\n        transmittance *= 1.0 - effectiveAlpha;");
fragment = replaceOnce(fragment, "        diskCrossingCount += 1;\n        if (transmittance < 0.02) break;", "        diskCrossingCount += 1;\n        if (transmittance < 0.02) { diagnosticTerminationReason = 4.0; break; }");
fragment = replaceOnce(fragment, "    previousSide = side;", "    pathLength += acceptedStepLength;\n    previousSide = side;");
fragment = replaceOnce(fragment, "  if (diskCrossingCount > 0) {", "  if (crossingDiagnosticMode > 3.5 && crossingDiagnosticMode < 4.5) {\n    outColor = vec4(vec3(diagnosticThirdReached), diagnosticThirdReached);\n    return;\n  }\n  if (crossingDiagnosticMode > 4.5 && crossingDiagnosticMode < 5.5) {\n    outColor = vec4(diagnosticThirdPreTrans, clamp(diagnosticThirdPreCoverage, 0.0, 1.0));\n    return;\n  }\n  if (crossingDiagnosticMode > 5.5) {\n    vec3 terminationColor = diagnosticThirdReached > 0.5 ? vec3(0.50) : terminationDiagnosticColor(diagnosticTerminationReason);\n    outColor = vec4(terminationColor, 1.0);\n    return;\n  }\n  if (crossingDiagnosticMode > 0.5) {\n    outColor = vec4(accumulatedDisk, clamp(diagnosticCoverage, 0.0, 1.0));\n    return;\n  }\n  if (diskCrossingCount > 0) {");
