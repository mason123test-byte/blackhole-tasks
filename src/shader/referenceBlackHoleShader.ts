...
float tangentConfigurationMomentumAreaLogRatio(
  float r,
  float srRight, float sthetaRight, float sprRight, float spthetaRight,
  float srUp, float sthetaUp, float sprUp, float spthetaUp
) {
  float safeR = max(r, 1.0);
  vec2 qRight = vec2(srRight / safeR, sthetaRight);
  vec2 qUp = vec2(srUp / safeR, sthetaUp);
  vec2 pRight = vec2(sprRight, spthetaRight / safeR);
  vec2 pUp = vec2(sprUp, spthetaUp / safeR);
  float configurationArea = abs(qRight.x * qUp.y - qRight.y * qUp.x);
  float momentumArea = abs(pRight.x * pUp.y - pRight.y * pUp.x);
  return clamp(log((configurationArea + 1e-6) / (momentumArea + 1e-6)), -8.0, 8.0);
}
...
void shapeIncidenceQualifiedDirectShoulder(
  ...
  float areaExchangeReversalSupport,
