...
float tangentReducedPhaseSpaceNorm(
  float r, float sr, float stheta, float spr, float sptheta
) {
  float safeR = max(r, 1.0);
  vec4 reducedTangent = vec4(sr / safeR, stheta, spr, sptheta / safeR);
  return max(length(reducedTangent), 1e-6);
}
...
void shapeIncidenceQualifiedDirectShoulder(
  ...
  float growthOrderPersistenceSupport,
