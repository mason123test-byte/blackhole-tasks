...
describe("black-hole render profiles", () => {
  ...
  it("keeps the validated diagnostic frame alive instead of destroying GL resources before Windows composition", () => {
    ...
    expect(rendererSource).toContain("const validatedSceneReady = getExpanded() < 0.5 || sceneReady;");
    expect(rendererSource).toContain("validatedEnergy = validatedSceneReady ? energy : 0;");
    expect(rendererSource).not.toContain("const validatedEnergy = validatedSceneReady ? energy : 0;");
