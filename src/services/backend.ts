...
export const backend = {
  async listTasks(): Promise<Task[]> { return isTauri() ? invoke("list_tasks") : read().tasks; },
  async listRelations(): Promise<TaskRelation[]> { return isTauri() ? invoke("list_relations") : read().relations; },
  async listTags(): Promise<Tag[]> { return isTauri() ? invoke("list_tags") : read().tags; },
  async createTask(input: CreateTaskInput): Promise<Task> {
    if (isTauri()) return invoke("create_task", { input });
  ...
  async updateTask(id: string, patch: Partial<Task>): Promise<Task> {
    if (isTauri()) return invoke("update_task", { id, patch });
  ...
  async deleteTask(id: string) {
    if (isTauri()) return invoke<void>("delete_task", { id });
  ...
  async completeTask(id: string): Promise<Task> {
    if (isTauri()) return invoke("complete_task", { id });
  ...
  async updatePositions(updates: TaskPositionUpdate[]) {
    if (isTauri()) return invoke<void>("update_tasks_positions", { updates });
  ...
  async createRelation(sourceTaskId: string, targetTaskId: string, relationType: TaskRelationType): Promise<TaskRelation> {
    if (isTauri()) return invoke("create_relation", { input: { sourceTaskId, targetTaskId, relationType, label: null } });
    const data = read();
  ...
  async deleteRelation(id: string) {
    if (isTauri()) return invoke<void>("delete_relation", { id });
    const data = read(); data.relations = data.relations.filter((item) => item.id !== id); write(data);
  },
  async getSettings(): Promise<AppSettings> { return isTauri() ? invoke("get_settings") : read().settings; },
  ...
  async window(command: string, args: Record<string, unknown> = {}) { if (isTauri()) await invoke(command, args); },
  async exportData(): Promise<string> {
    if (isTauri()) return invoke("export_data");
