...
export function BlackHoleCanvas({ hovered, expanded, pulse, quality, lowPowerMode, onError }: { hovered: boolean; expanded: boolean; pulse: number; quality: RenderQuality; lowPowerMode: boolean; onError?(message: string): void }) {
  ...
  useEffect(() => { if (!ref.current) return; return startBlackHole(ref.current, () => hoverRef.current, () => pulseRef.current, () => expandedRef.current, { quality, lowPowerMode, onError }); }, [lowPowerMode, onError, quality]);
