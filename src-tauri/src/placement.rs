...
pub fn calculate(input: PlacementInput) -> PlacementResult {
    ...
    let fallback = [
        (
            "right",
            right.saturating_sub(gap) as i64 * area.height as i64,
            4_i64,
        ),
        (
            "left",
            left.saturating_sub(gap) as i64 * area.height as i64,
            3_i64,
        ),
        (
            "bottom",
            below.saturating_sub(gap) as i64 * area.width as i64,
            2_i64,
        ),
        (
            "top",
            above.saturating_sub(gap) as i64 * area.width as i64,
            1_i64,
        ),
    ]
    .into_iter()
